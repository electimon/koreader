...
local Device = {
    ...
    hasFewKeys = no,
    hasWifiToggle = no,
    hasSeamlessWifiToggle = no, -- Can toggle Wi-Fi without focus loss and extra user interaction (i.e., not Android)
    hasWifiManager = no,
    hasWifiRestore = no,
    ...
    canUseCBB = no, -- The C BB maintains a 1:1 feature parity with the Lua BB, except that is has NO support for BB4, and limited support for BBRGB24
    hasColorScreen = yes,
...
function Device:init()
    ...
    logger.info("past framebuffer init")

    self.screen.isColorScreen = self.hasColorScreen
    self.screen.isColorEnabled = function() return true end
    ...
    logger.info("past viewport setup")
    ...
    logger.info("past screen size setup")
    ...
    logger.info("past screen DPI setup")
